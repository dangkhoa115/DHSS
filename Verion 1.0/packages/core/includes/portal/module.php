...
class Module
{
	...
	static function get_setting($name,$default=false, $block_id = false)
	{
		if($block_id)
		{
			if($block_id<1000)
	...
	static function set_setting($setting_id,$value)
	{
	...
	static function get_help_topic_id()
	{
	...
	static function convert_language($layout)
	{
		eval('?>'. preg_replace('/\[\[\.(\w+)\.\]\]/','<?php echo Portal::language(\'\\1\');?>',$layout).'<?php ');
	}
	
	static function make_ext_region($region, $container_id=0, $baseCls=false)
	{
		global $blocks;
		$first = true;
		$st = '';
