	...
	if(key==40 || key==38){
		...
		if(key==37 && pos==0){
			...
			for(var i=0;i<inputs.length;i++){
				if(inputs[i].id==name+'_'+index){
					found=true;
				} else if(found && typeof(inputs[i].name)!='undefined' && inputs[i].id.indexOf('_'+index)!=-1 && inputs[i].type!="hidden" && inputs[i].style.display=='' && !inputs[i].disabled){
					inputs[i].focus();
					break;
				}
			}
...
function mi_delete_row(obj,name,index,prefix){
	...
	if(getId(prefix+'deleted_ids') && getId('id_'+index).value && getId('id_'+index).value!='(auto)'){
		getId(prefix+'deleted_ids').value+=(getId(prefix+'deleted_ids').value?',':'')+getId('id_'+index).value;
	}
...
function mi_init_row(name,value){
	...
	for(var i in value){
		if(getId(i+'_'+input_count)){
			...
			} else if(getId(i+'_'+input_count).tagName == 'SPAN' || getId(i+'_'+input_count).tagName == 'DIV'){
				getId(i+'_'+input_count).innerText = value[i];
			} else if(getId(i+'_'+input_count).tagName == 'file'){
			
			} else {
				if(getId('img_'+i+'_'+input_count) && typeof(getId('img_'+i+'_'+input_count).src) != 'undefined'){
					if(value[i]){
