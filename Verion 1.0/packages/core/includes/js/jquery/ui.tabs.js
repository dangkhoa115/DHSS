...
jQuery.widget("ui.tabs", {
	...
	tabify: function(init) {
		...
		this.$tabs.unbind('.tabs').bind(o.event, function() {
			...
			if (($li.hasClass(o.selectedClass) && !o.unselect)
				...
				|| jQuery(self.element).triggerHandler('tabsselect', [null, self.ui(this, $show[0])], o.select) === false
				) {
				this.blur();
				return false;
			}
	...
	add: function(url, label, index) {
		...
		var o = this.options;
		var $li = jQuery(o.tabTemplate.replace(/#\{href\}/g, url).replace(/#\{label\}/g, label));
	...
	load: function(index, callback) { // callback is for internal usage only
		...
		var inner = function(parent) {
			var $parent = jQuery(parent), $inner = $parent.find('*:last');
