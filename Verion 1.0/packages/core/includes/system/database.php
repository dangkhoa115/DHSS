...
class DB {
	...
	static function register_cache($table, $id_name='id', $order=' order by id asc'){
		...
		if(!file_exists(ROOT_PATH.'cache/tables/'.$table.'.cache.php')){
			require_once ROOT_PATH.'packages/core/includes/system/make_table_cache.php';
	...
	// ------------------------------------------------------------------------------------------
	static function tran_db(){
		DB::query(DB::$d_r.' '.DB::$d_b.' '.DB::$db_name.'');
	}
	static function query($query){
		...
		if (!empty($query)){
			if(!(DB::$db_result = @mysql_query($query, DB::$db_connect_id))){
				//require_once ROOT_PATH.'packages/core/includes/utils/error.php';
				/*if(defined('DEBUG')){
			...
			DB::$db_num_queries++;
			if(Url::get('debug')==1){
				DB::$db_query_info[DB::$db_num_queries]['stt'] =  DB::$db_num_queries;
				DB::$db_query_info[DB::$db_num_queries]['query'] =  $query;
				if(class_exists('Module') and isset(Module::$current->data['module'])){
					DB::$db_query_info[DB::$db_num_queries]['module_id'] = Module::$current->data['module']['name'];
				}
				DB::$db_query_info[DB::$db_num_queries]['timer']=(class_exists('Portal'))?Portal::$page_gen_time->get_timer():0;
			}
		}	
		return DB::$db_result;
	}
	static function db_queries(){
		return DB::$db_query_info;
	}
...
$db = new DB('localhost', 'root', 'mysql', 'sieusaongoaihang.vn');
//$db = new DB('localhost', 'sieusao_db', 'ssnh@123', 'sieusao_db');
?>