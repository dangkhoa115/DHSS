...
{
	$sql = 'SELECT * FROM '.$table.' WHERE structure_id = '.(($id != ID_ROOT)?IDStructure::parent($id):ID_ROOT).$extra;
	DB::query($sql);
	$row = DB::fetch();
	return $row;
}
...
{
	$parent = si_parent($table,DB::structure_id($table,Url::iget('id')),$extra);
	$category=DB::exists_id($table,Url::iget('id'));
