...
function update_upload_file($field, $dir,$type='IMAGE',$old_file=false,$new_width=false,$new_height=false, $constraint=false){
	if(isset($_FILES[$field]) and $_FILES[$field]['name']){
		...
		if(preg_match('/\.('.$type_file.')$/i',strtolower($_FILES[$field]['name']),$matches) and filesize($_FILES[$field]['tmp_name'])< $max_upload_file_size){
			//echo $_FILES[$field]['tmp_name'].'-'.$_REQUEST[$field];
			if(move_uploaded_file($_FILES[$field]['tmp_name'],$_REQUEST[$field])){
				if($new_width and $new_height){

					create_thumb($_REQUEST[$field],$_REQUEST[$field],$new_width,$new_height, true);//25/06/2015
					//create_thumb($_REQUEST[$field],$thumb_name,$new_width,$new_height, true);
					//create thumb//$constraint//$new_width,$new_height
...
function multi_upload_file($field, $dir,$type='IMAGE'){
	if(isset($_FILES[$field])){	
		$new_name = '';
		if(!is_dir('upload/'.substr(PORTAL_ID,1))){
			@mkdir('upload/'.substr(PORTAL_ID,1));			
		}
		...
		foreach($_FILES[$field]['name'] as $k=>$value){
			if($value){
				if(file_exists('upload/'.$dir.'/'.$value)){				
					$new_name[$k+1]['value'] = 'upload/'.$dir.'/'.time().'_'.$value;					
				}else{
					require_once 'packages/core/includes/utils/vn_code.php';	
					$new_name[$k+1]['value'] = 'upload/'.$dir.'/'.convert_utf8_to_latin($value);
				}
