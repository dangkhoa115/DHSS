...
class EditPackageAdminForm extends Form
{
	function EditPackageAdminForm()
	{
		...
		$this->add('name',new TextType(true,'invalid_name',0,255)); 
			$this->add('title_1',new TextType(false,'invalid_title',0,2000)); 
			$this->add('description_1',new TextType(false,'invalid_description',0,200000)); 
	...
	function on_submit()
	{
		...
		{
			$extra = array();
			$extra=$extra+array('title_1'=>Url::get('title_1',1)); $extra=$extra+array('description_1'=>Url::get('description_1',1)); 
			$new_row = $extra+
				array(
					'name', 'type'
				);
