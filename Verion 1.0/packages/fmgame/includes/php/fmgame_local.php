...
class FMGAME{
	...
	static function nhan_qua_hlv(){
		...
		if($diem_kn>=3000){
			if(!DB::exists('select id from fmg_received_igold where account_id="'.$account_id.'" and diem_kn=2000')){
				DB::insert('fmg_received_igold',array('account_id'=>$account_id,'diem_kn'=>3000,'igold'=>10,'time'=>time()));			
				iGold::receive_igold($account_id,50,'Starteam: Thưởng điểm kinh nghiệm HLV mức 3000');
				$kq = 'true';
			}else{
				$kq = '3000';
			}
	...
	static function get_anh_hang_hlv($account_id){
		...
		}elseif($diem_kn>=1000 and $diem_kn<3000){	
	...
	static function get_vong_dau_trong_ngay($server_id,$play_off=0){
		echo $sql = '
			...
			where 
				fmg_vong_dau.server_id='.$server_id.'
				AND fmg_schedule.thoi_gian <= "'.date('Y-m-d H:s:i').'"
	...
	static function get_transfer_time($du_bi=false){
		if($du_bi){
			return 3;
		}else{
			$diem_kn = FMGAME::get_diem_kn(Session::get('user_id'));
			if($diem_kn>=2000){
				return 4; 
			}else{
				return 3; 
	...
	static function get_total_transfer_time($clb_id,$server_id,$du_bi=false){
		return DB::fetch('select count(*) as acount from fmg_clb_transfer where clb_id='.$clb_id.' and vong_dau_id='.$server_id.' and sell = 0 and create_team=0 '.($du_bi?' and du_bi=1':' and du_bi=0').'','acount');
	}
	static function can_transfer(){
		...
		$transfer_time = FMGAME::get_transfer_time() + FMGAME::get_transfer_time(true);
		$clb_id = FMGAME::my_team_id($mua_giai_id);
		if(DB::fetch('select `full` from fmg_clb where id='.$clb_id.'','full')==1){
			if(((date('N')>=3 and date('N') <= 5) or (date('N')==6 and date('H')<= 20)))// or $_SERVER['REMOTE_ADDR'] == '1.55.96.24'
			{
				...
				if(FMGAME::get_total_transfer_time($clb_id,$server_id) + FMGAME::get_total_transfer_time($clb_id,$server_id,true) >= $transfer_time + FMGAME::get_tong_so_the_cn()){
					return false;
				}else{
					return true;
				}
	...
	static function auto_generate_clbs(){
		$array = array(
			...
			'Hold me forever',
			'Sivir','Mogana','Final stage','Microwaves'
		);
		for($i=0;$i<19;$i++){
			$ten_doi = $array[$i];
	...
	static function get_vi_tri($cau_thu_id){
		$sql = '
			select 
	...
	static function get_team($clb_id){
		$sql = '
			...
			from 
				fmg_clb
				LEFT OUTER JOIN account on account.id=fmg_clb.account_id
	...
	static function get_team_cost($mua_giai_id=2){
		...
		$sql = '
			SELECT
				fmg_clb_cau_thu.id,vttn.cost
			FROM
				ssnh_vi_tri_theo_nam AS vttn
				inner join ssnh_cau_thu ON ssnh_cau_thu.id = vttn.cau_thu_id
				inner join  fmg_clb_cau_thu ON ssnh_cau_thu.id = fmg_clb_cau_thu.cau_thu_id
			WHERE
	...
	static function get_pos_count_temp($ma_vi_tri=false,$du_bi=false){
		$total = 0;
		if(!isset($_SESSION['clb_tmp']['cau_thus'])){
			...
			if($ma_vi_tri){
				foreach($cau_thus as $value){
					...
					if(DB::exists($sql)){
						if($du_bi){
							if(isset($value['du_bi']) and $value['du_bi']){
								$total++;
							}
						}else{
							if(!isset($value['du_bi']) or !$value['du_bi']){
								$total++;
							}
						}
					}
...
//				and fmg_server.status="OPEN"
		$servers = DB::fetch_all($sql);
		$id_vong_dau_ket_thuc_gan_nhat = get_id_vong_dau_ket_thuc_gan_nhat(MUA_GIAI_ID,true);
			...
			foreach($vong_daus as $vk=>$vv){
				...
				if(!empty($cap_daus)){
					foreach($cap_daus as $key=>$val){
						$kq_arr = FMGAME::play_ty_so($val['dcn_id'],$val['dkh_id'],$id_vong_dau_ket_thuc_gan_nhat);	
						$kq = 'Hòa';
						$ty_so = '0:0';
						$ghi_ban1 = '';
						$ghi_ban2 = '';
						...
						/*-------*/
						if(!empty($kq_arr)){
							$ghi_ban = isset($kq_arr['ghi_ban'])?$kq_arr['ghi_ban']:'';
							if(isset($kq_arr[$val['dcn_id']])){
								//System::Debug($kq[$clb_id1]);
								foreach($kq_arr[$val['dcn_id']] as $key1=>$val1){
									//echo intval($val['ban_thang']);
									$ts1 += intval($val1['ban_thang']);
									if($ts1){
										$ghi_ban1 .= ($ghi_ban1?'<br>':'').$val1['name'].' ('.$val1['ban_thang'].' bàn)';
									}
								}
							}
							$ty_so = $ts1.':';
							if(isset($kq_arr[$val['dkh_id']])){
								foreach($kq_arr[$val['dkh_id']] as $key2=>$val2){
									$ts2 += intval($val2['ban_thang']);
									if($ts2){
										$ghi_ban2 .= ($ghi_ban2?'<br>':'').$val2['name'].' ('.$val2['ban_thang'].' bàn)';
									}
								}
							}
							$ty_so .= $ts2;
						...
						/*
						$win_arr = array(6,13,5,15,1,24,312,9,14);
						//592,94,91,41,19,30,29,35,87,85,52,59,57,752
						...
						$loser_arr = array(180,304,594,769,768,226,600,675); // neft game/ 804,755 chi neft 1 tran
						...
						///////////////////////////*/
						$arr = array('diem_doi_chu_nha'=>$diem_doi_chu_nha,'ket_qua'=>$ket_qua,'diem_doi_khach'=>$diem_doi_khach,'ban_thang'=>$ts1,'ban_thua'=>$ts2,'ghi_ban'=>($ghi_ban1.':'.$ghi_ban2));						
						if($key){
							DB::update('fmg_schedule',$arr,'id='.$key);
							///////////////////////////update vi tri cua server hien tai////////
							$t1 = FMGAME::get_power_clb_quickly($val['dcn_id'],$id_vong_dau_ket_thuc_gan_nhat,MUA_GIAI_ID);
							...
							$t2 = FMGAME::get_power_clb_quickly($val['dkh_id'],$id_vong_dau_ket_thuc_gan_nhat,MUA_GIAI_ID);
	...
	static function get_stamina($clb_id){
		$sta = DB::fetch('select stamina from fmg_clb where id='.$clb_id.'','stamina');
		return intval($sta); 
	}
	...
	static function play($clb_id1,$clb_id2,$vong_dau_id,$kick_off=false){
		...
		$t1 = $t1 + ($t1_o?$sta1:0); // edited 29/12
		$t2 = $t2 + ($t2_o?$sta2:0); // edited 29/12
	...
	static function play_ty_so($clb_id1,$clb_id2,$vong_dau_id,$kick_off=false){
		$mua_giai_id = MUA_GIAI_ID;
		$kt_vong_dau_id = get_id_vong_dau_ket_thuc_gan_nhat(MUA_GIAI_ID,true);
		$tong_diem = 0;
		$power1 = FMGAME::get_power_clb_quickly($clb_id1,$vong_dau_id,$mua_giai_id);
		$power2 = FMGAME::get_power_clb_quickly($clb_id2,$vong_dau_id,$mua_giai_id);
		$per1 = ($power1 + $power2)?round(($power1/($power1 + $power2))*100):0;
		$per2 = ($power1 + $power2)?round(($power2/($power1 + $power2))*100):0;
		$sql = '
			select 
			...
			where 
				fmg_clb_cau_thu.clb_id = '.$clb_id1.'
				AND lsct.diem>0
				AND (fmg_clb_cau_thu.sell_vong_dau_id = 0 or sell_vong_dau_id > '.$kt_vong_dau_id.')
				AND (fmg_clb_cau_thu.vong_dau_id <=  '.$kt_vong_dau_id.')
			...
			LIMIT
				0,100
		';
		$cauthu1 = DB::fetch_all($sql);	
		$tong_ban_thang1 = 0;
		$result = array();
		foreach($cauthu1 as $key=>$value){
			$phong_do_cau_thu = (rand(1,5)*10*2)/100;
			$diem = $value['diem'];	
			if($value['ma_vi_tri'] == 'TM'){
				$xac_suat = 0;
			}elseif($value['ma_vi_tri'] == 'HV'){
				$xac_suat = 20/100;
			}elseif($value['ma_vi_tri'] == 'TV'){
				$xac_suat = 30/100;
			}elseif($value['ma_vi_tri'] == 'TD'){
				$xac_suat = 50/100;
			}
			/*$diem_kn = FMGAME::get_diem_kn($value);
			if($diem_kn<500){
				$str = 'Tập sự';
			}elseif($diem_kn>=500 and $diem_kn<1000){
				$str = 'Bán chuyên';	
			...
			//if($ban_thang = floor(($phong_do_cau_thu*($diem/10)*$xac_suat*($per1/100))*10))
			if($ban_thang = floor(($phong_do_cau_thu*($diem/10)*$xac_suat*($per1/100))*10))
			{
				$result[$clb_id1][$key] = array('id'=>$key,'name'=>$value['ten'],'ban_thang'=>$ban_thang);
			}
		}
		$sql = '
			select 
			...
			from
				...
				inner join ssnh_vong_dau ON ssnh_vong_dau.id = lsct.vong_dau_id
			where 
				fmg_clb_cau_thu.clb_id = '.$clb_id2.'
				AND lsct.diem>0
				AND (fmg_clb_cau_thu.sell_vong_dau_id = 0 or sell_vong_dau_id > '.$kt_vong_dau_id.')
				AND (fmg_clb_cau_thu.vong_dau_id <=  '.$kt_vong_dau_id.')
				'.($vong_dau_id?'AND ssnh_vong_dau.id = '.$vong_dau_id.'':'').'
			...
			LIMIT
				0,100
		';
		$cauthu2 = DB::fetch_all($sql);	
		$tong_ban_thang2 = 0;
		foreach($cauthu2 as $key=>$value){
			$phong_do_cau_thu = (rand(1,5)*10*2)/100;
			$diem = $value['diem'];	
			if($value['ma_vi_tri'] == 'TM'){
				$xac_suat = 0;
			...
			if($ban_thang = floor(($phong_do_cau_thu*($diem/10)*$xac_suat*($per1/100))*10))
			{
				$result[$clb_id2][$key] = array('id'=>$key,'name'=>$value['ten'],'ban_thang'=>$ban_thang);
			}
		}
		return $result;
	}
	...
	static function get_ls_cnh($clb_id,$vong_dau_id=false,$buy=false,$item_per_page=100){
		...
		$sql = '
			...
			where 
				'.$cond.'
			order by 
				fmg_clb_transfer.du_bi,fmg_clb_transfer.time DESC
			LIMIT
				0,'.$item_per_page.'
		';
		$items = DB::fetch_all($sql);
	...
	static function get_dubis($clb_id,$vong_dau_id){
		$mua_giai_id = MUA_GIAI_ID;
		$kt_vong_dau_id = get_id_vong_dau_ket_thuc_gan_nhat(MUA_GIAI_ID,true);
		$next_vong_dau_id = get_id_vong_dau_hien_tai(true);
		$cond = '
				fmg_clb_cau_thu.du_bi = 1
				AND fmg_clb_cau_thu.clb_id = '.$clb_id.'
				AND ssnh_cau_thu_clb.mua_giai_id = '.$mua_giai_id.'
		';
		$cond .= '
	...
	static function get_cauthus($clb_id,$vong_dau_id,$ma_vi_tri=false,$get_by_cau_thu_id=false,$du_bi=false){
		$mua_giai_id = MUA_GIAI_ID;
		$kt_vong_dau_id = get_id_vong_dau_ket_thuc_gan_nhat(MUA_GIAI_ID,true);
		$next_vong_dau_id = get_id_vong_dau_hien_tai(true);
		$cond = '
				'.($du_bi?'1=1':'fmg_clb_cau_thu.du_bi = 0').'
				AND fmg_clb_cau_thu.clb_id = '.$clb_id.'
		...
		$sql = '
			select 
				...
				ssnh_cau_thu.id as cau_thu_id,
				ssnh_cau_thu.name_id,
				'.($get_by_cau_thu_id?'CONCAT(vtts.ten," ",ssnh_cau_thu.ten) as ten,':'ssnh_cau_thu.ten,').'
				ssnh_cau_thu.so_ao,
	...
	static function cache_power($vong_dau_id){
		...
		$clbs = DB::fetch_all($sql);
		foreach($clbs as $k=>$v){
			DB::update('fmg_clb',array('cache_power'=>FMGAME::get_power_clb_quickly($k,$vong_dau_id,MUA_GIAI_ID)),'id='.$k);
		}
	}
	static function get_power_clb_quickly($clb_id,$vong_dau_id=false,$mua_giai_id=2){
		$kt_vong_dau_id = get_id_vong_dau_ket_thuc_gan_nhat($mua_giai_id,true);
		$tong_diem = 0;
		$sql = '
			select 
				sum(IF(`captain`= 1,(lsct.diem + lsct.bonus)*2,lsct.diem + lsct.bonus)) as diem
			from
				ssnh_lich_su_cau_thu AS lsct 
				inner join ssnh_cau_thu ON ssnh_cau_thu.id = lsct.cau_thu_id
				inner join fmg_clb_cau_thu ON fmg_clb_cau_thu.cau_thu_id = ssnh_cau_thu.id
				inner join ssnh_cau_thu_clb on ssnh_cau_thu_clb.cau_thu_id = ssnh_cau_thu.id
				inner join ssnh_vong_dau ON ssnh_vong_dau.id = lsct.vong_dau_id
			where 
				fmg_clb_cau_thu.clb_id = '.$clb_id.'
		...
		';
		$power =  DB::fetch($sql,'diem');		
		$ao = FMGAME::get_items($clb_id,DP_AO);
		if($ao){
			$power += 2;
		}
		$quan = FMGAME::get_items($clb_id,DP_QUAN);
		if($quan){
			$power += 1;
		}
		$tat = FMGAME::get_items($clb_id,DP_TAT);
		if($tat){
			$power += 1;
		}
		$giay = FMGAME::get_items($clb_id,DP_GIAY);
		if($giay){
			$power += 2;
		}
		return $power;
	...
	static function get_power_clb($clb_id,$vong_dau_id=false,$mua_giai_id=2,$type='ALL'){
		...
		foreach($items as $key=>$value){
			//$phong_do_cau_thu = (rand(1,5)*10*2)/100;
			$value['diem'] = $value['captain']?($value['diem']*2):$value['diem'];	
			if(($type=='ALL' or $type=='PN') and ($value['ma_vi_tri'] == 'TM' or $value['ma_vi_tri'] == 'HV')){
				$phong_ngu += $value['diem'];
			}
			if(($type=='ALL' or $type=='TC') and $value['ma_vi_tri'] == 'TV'){
				$tien_ve += $value['diem'];
			}
			if(($type=='ALL' or $type=='TC') and $value['ma_vi_tri'] == 'TD'){
				$tan_cong += $value['diem'];
			}
		}
		if($type=='ALL' or $type=='PN'){
			if($ao){
				$phong_ngu += 2;
			}
			if($quan){
		...
		$tong_diem = $phong_ngu + $tien_ve + $tan_cong;
		return $tong_diem;
	}
	...
	static function get_lichthidaus($vong_dau_id=false,$item_per_page=10,$cond=false,$order='kqtd.thoi_gian'){
		...
		$sql = '
					select 
						...
						kqtd.ban_thang,
						kqtd.ban_thua,
						CONCAT(kqtd.ban_thang,":",kqtd.ban_thua) AS ty_so,
	...
	function get_bangxephang($server_id){
		...
		$sql = '
			select 
				...
				(select SUM(kqtd1.diem_doi_chu_nha) from fmg_schedule as kqtd1 where kqtd1.doi_chu_nha_id = fmg_clb.id and kqtd1.server_id='.$server_id.') as diem_chunha,
				(select SUM(kqtd2.diem_doi_khach) from fmg_schedule as kqtd2 where kqtd2.doi_khach_id = fmg_clb.id and kqtd2.server_id='.$server_id.') as diem_khach,
				(select SUM(kqtd4.ban_thang) from fmg_schedule as kqtd4 where kqtd4.doi_khach_id = fmg_clb.id and kqtd4.server_id='.$server_id.')
				+ (select SUM(kqtd4.ban_thang) from fmg_schedule as kqtd4 where kqtd4.doi_chu_nha_id = fmg_clb.id and kqtd4.server_id='.$server_id.')
				 as ban_thang,
				(select SUM(kqtd5.ban_thua) from fmg_schedule as kqtd5 where kqtd5.doi_khach_id = fmg_clb.id and kqtd5.server_id='.$server_id.')
				+(select SUM(kqtd5.ban_thua) from fmg_schedule as kqtd5 where kqtd5.doi_chu_nha_id = fmg_clb.id and kqtd5.server_id='.$server_id.')
				 as ban_thua
			from 
				fmg_clb
				inner join fmg_clb_server ON fmg_clb_server.clb_id = fmg_clb.id
		...
		if(!empty($items)){
			...
			array_multisort($sort['diem'], SORT_DESC,$sort['hieu_so'], SORT_DESC,$sort['power'],SORT_ASC,$items);
			$i=1;
			foreach($items as $key=>$value){
	...
	static function schedule($server_id){
		$d = 'd';
		$mua_giai_id = MUA_GIAI_ID;
		$vong_daus_temp = FMGAME::get_vongdaus($server_id,false);
		//System::debug($vong_daus_temp);die;
		$cond = '
			fmg_clb.time < '.(strtotime(date('Y-m-'.$d)) + (20*3600)).'
		';//AND fmg_clb.time < '.(strtotime('2016-02-27') + 20*3600).'
		//fmg_clb.cache_power > 0
		//	AND fmg_clb.time < '.(strtotime('2015-02-27') + 20*3600).'
		...
		if($tong_clb>= MAX_TEAM and DB::fetch('select count(*) as total from fmg_schedule where server_id='.$server_id.'','total')<$tong_clb*($tong_clb-1)){
			...
			//if((date('N')==6 and date('H')<20))
			{
				...
				$start_time = strtotime(date('2016-04-19 00:00:00'));// dung de test
				$t = $start_time;
				$games = array();
	...
	static function create_server($d = 1,$m=12){
		...
		$total_clb = DB::fetch('select count(fmg_clb.id) as acount from fmg_clb where cache_power > 0 and train > 0','acount');
		$time = time();
		$mua_giai_id = MUA_GIAI_ID;
		...
		for($i=1;$i<=$total;$i++){
			...
			for($j=1;$j<=38;$j++){
				if(!DB::exists('select id from fmg_vong_dau where server_id='.$server_id.' and ten="Vòng '.$j.'"')){
					DB::insert('fmg_vong_dau',array(
						'server_id'=>$server_id,
						'ten'=>'Vòng '.$j
					));
				}
	...
	static function auto_lien_dau(){
		...
		foreach($servers as $k=>$v){
			...
			foreach($cap_daus as $key=>$val){
				...
				if($val['clb_id1'] == 1046){//803 1 vong
					$win_clb_id = $val['clb_id1'];
				}
				if($val['clb_id2'] == 1046){
					$win_clb_id = $val['clb_id2'];
	...
	static function get_kq($clb1,$clb2,$vong_dau_id,$champion=false){
		if($champion){
			...
			SELECT
				...
				`fmg_schedule`.`ban_thang`,
				`fmg_schedule`.`ban_thua`,
				CONCAT(`fmg_schedule`.`ban_thang`,":",`fmg_schedule`.`ban_thua`) AS ty_so,
				`fmg_schedule`.`ghi_ban`,
	...
	static function delete_cau_thu_temp($cau_thu_id){
		...
		foreach($cau_thus as $key=>$val){
			...
			if($cau_thu_id==$val['cau_thu_id']){
				if(!isset($val['tmp'])){
					$cau_thu = DB::fetch('select id,ten,anh_dai_dien from ssnh_cau_thu where id='.$cau_thu_id.'');
					$val['ten'] = $cau_thu['ten'];
					$val['anh_dai_dien'] = $cau_thu['anh_dai_dien'];
