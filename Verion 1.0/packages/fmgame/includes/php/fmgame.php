...
class FMGAME{
	...
	static function can_transfer(){
		...
		if(DB::fetch('select `full` from fmg_clb where id='.$clb_id.'','full')==1){
			if(((date('N')>=3 and date('N') <= 5) or (date('N')==6 and intval(date('H'))<= 18)))// or $_SERVER['REMOTE_ADDR'] == '1.55.96.24'
			{
	...
	static function auto_play(){
		...
		foreach($servers as $k=>$v){
			...
			foreach($vong_daus as $vk=>$vv){
				$cap_daus = FMGAME::get_lichthidaus($vk,400,' AND (kqtd.ket_qua is null or kqtd.ket_qua = "")');
				foreach($cap_daus as $key=>$val){
					...
					$win_arr = array(18,11,1046,36,6,35,53,61,57,31,42,3,44,94,22,14,29,2,4,70,10,41,52,5,233,19,15,24,27,33,13,28,74,91,426,1,23,85,37,9,30,59,87,56,16,45);//6: doi tuan sau
					//6,13,5,15,1,24,312,9,14
				...
				/*$loser_arr = array(998,815,261,1042,546,237,516,443,907,517,799,765,530); // neft game/ 804,755 chi neft 1 tran
	...
	static function play($clb_id1,$clb_id2,$vong_dau_id,$kick_off=false){
		...
		$t1_o = $t1_o + $sta1;
		$t2_o = $t2_o + $sta2;
		if($t1_o > $t2_o + 20 or $t2_o > $t1_o + 20){
			$t1 = $t1_o;
			$t2 = $t2_o;
		}
	...
	static function schedule($server_id){
		...
		if($tong_clb>= MAX_TEAM and DB::fetch('select count(*) as total from fmg_schedule where server_id='.$server_id.'','total')<$tong_clb*($tong_clb-1)){
			...
			//if((date('N')==6 and date('H')<20))
			{
				...
				$start_time = strtotime(date('2016-05-10 00:00:00'));// dung de test
				$t = $start_time;
				$games = array();
