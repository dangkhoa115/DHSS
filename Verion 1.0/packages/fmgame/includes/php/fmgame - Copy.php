...
class FMGAME{
	...
	static function get_running_server($mua_giai_id=2,$max=false,$cond=false,$limit=1000){
		$sql = '
			...
			where 
				...
				'.($max?' AND (select count(*) from fmg_clb where fmg_clb.server_id=fmg_server.id)<'.MAX_TEAM.'':'').'
				'.$cond.'
			order by
				tong_clb DESC
			limit
	...
	static function can_transfer(){
		...
		if(DB::fetch('select full from fmg_clb where id='.$clb_id.'','full')==1){
			if((date('N')==6 and date('H')>=14))
			{
				...
				if(DB::fetch('select count(*) as acount from fmg_clb_transfer where clb_id='.$clb_id.' and vong_dau_id='.$vong_dau_id.' and create_team=0','acount') >=3){
					return false;
				}else{
					return true;
				}
	...
	static function get_clbs($server_id,$mua_giai_id,$order_by='fmg_clb.name',$limit=1000){
		$sql = '
			select 
	...
	static function auto_generate_clbs(){
		$array = array('Dream Team','Super Stars','The Angels','Super Team','Nhà vô địch','The men','xMen','Tôi yêu bóng đá','Những người bạn','Hot girls');
		for($i=0;$i<10;$i++){
			$ten_doi = $array[$i];
	...
	static function get_team($clb_id){
		$sql = '
			select 
				fmg_clb.id,fmg_clb.logo,fmg_clb.name,fmg_clb.stamina,fmg_clb.first_time
			from 
				fmg_clb
			where
				fmg_clb.id='.$clb_id.'
		';
		$item = DB::fetch($sql);
		$item['server'] = DB::fetch('select fmg_server.name from fmg_server inner join fmg_clb_server on fmg_clb_server.server_id=fmg_server.id where fmg_clb_server.clb_id='.$clb_id.' and fmg_server.status<>"CLOSED"','name');
	...
	static function get_phong_do($clb_id){
		///////////////////////
		$phong_do = '';
		$sql = 'select id,doi_chu_nha_id,diem_doi_chu_nha,doi_khach_id,diem_doi_khach from fmg_schedule where (ket_qua!="") and (doi_chu_nha_id='.$clb_id.' or doi_khach_id='.$clb_id.') order by id desc limit 0,10';
	...
	static function get_team_cost($mua_giai_id=2){
		$clb_id = Url::iget('clb_id')?Url::iget('clb_id'):FMGAME::my_team_id($mua_giai_id);
		...
		$sql = '
			...
			WHERE
				...
				AND fmg_clb_cau_thu.account_id = "'.$user_id.'"
	...
	static function play($clb_id1,$clb_id2,$vong_dau_id,$kick_off=false){
		...
		$thang1 = round(($tc1/($t1_o + $t2_o)*10)*3);
		$t1 = $t1 + ($thang1 - $thua1);
		$thua2 = $pn2?(round(($t1_o + $t2_o)/$pn2)):10;
		$thang2 = round(($tc2/($t1_o + $t2_o)*10)*3);
		$t2 = $t2 + ($thang2 - $thua2);
		////////////////////////////////////////////////////////////////////
	...
	static function get_cauthus($clb_id,$vong_dau_id,$ma_vi_tri=false,$mua_giai_id=2){
		$sql = '
			select 
				fmg_clb_cau_thu.id,
	...
	function get_bangxephang($server_id){
		...
		$vong_dau_id = get_id_vong_dau_ket_thuc_gan_nhat();
		$sql = '
			select 
			...
			if($total_tran==0 or $total_tran<$tong_clb*($tong_clb-1)){
				if($v['tong_clb']>=MAX_TEAM){
					if($v['tong_clb']%2==0){
						...
						$temp = FMGAME::get_running_server($mua_giai_id,true,' AND fmg_server.status="OPEN" AND fmg_server.id<>'.$server_id.' and '.MAX_TEAM.'>(select count(fmg_clb_server.id) from fmg_clb_server where fmg_clb_server.server_id=fmg_server.id)',1);
						$temp = array_shift($temp);
						$old_server_id = $temp['id'];
						$clb = FMGAME::get_clbs($old_server_id,$mua_giai_id,'fmg_clb.name',1);
					...
					/////////get clb da ton tai/////////
					$existed_clb_ids = '';
					$tmp = DB::fetch_all('select fmg_clb_server.clb_id as id from fmg_clb_server inner join fmg_server ON fmg_server.id=fmg_clb_server.server_id where fmg_server.status="OPEN"');
	...
	static function schedule($server_id){
		...
		$clbs_temp = FMGAME::get_clbs($server_id,$mua_giai_id);
		$clbs = array();
		$i=1;
		...
		if($tong_clb>= MAX_TEAM and DB::fetch('select count(*) as total from fmg_schedule where server_id='.$server_id.'','total')<$tong_clb*($tong_clb-1)){
			$i=1;
			if((date('N')==2 and date('H')<=8))
			{
				$start_time = strtotime(date('Y-m-d 00:00:00'));
				//$start_time = strtotime(date('2015-10-06 00:00:00'));// dung de test
				$t = $start_time;
				$games = array();
					...
					for( $j=1; $j<=$teams; $j++ ) {
						...
						if($vd!=-1){
							if(isset($vong_daus[$vd]) and $vong_dau_id=$vong_daus[$vd]){
								$day = floor(($vd-1)/10);
								$t = $start_time +$day*24*3600 + ($vd-1)*3600;
								if($vd>1 and $vd%10==0){
									$h=18;
								}else{
			...
			if($round<$total_rounds)
			{
				...
				foreach($cap_daus as $key=>$val){
					...
					if($cap_dau = DB::fetch('select fmg_lien_dau.id,fmg_clb.logo,fmg_lien_dau.win_clb_id,fmg_lien_dau.round,fmg_clb.name from fmg_lien_dau inner join fmg_clb on fmg_clb.id = fmg_lien_dau.win_clb_id where fmg_lien_dau.round='.($round-1).' and fmg_lien_dau.ld_server_id='.$server_id.' and fmg_lien_dau.win=1')){
						...
						$str .= '<div class="lien-dau-team vo-dich">';
						$str .= '<img class="cup" src="skins/ssnh/images/fm_game/win.png"><span>AI SẼ LÀ NHÀ VÔ ĐỊCH?</span>';
						$str .= '</div>';
					}
					for($c=0;$c<(($num_teams/2-1)*4/2);$c++){
						$str .= '<div class="lien-dau-team space">';
	...
	static function auto_lich_lien_dau(){
		...
		{// ngay thu 7
			$mua_giai_id = MUA_GIAI_ID;
			if($row=DB::fetch('select id from fmg_server where for_winner=1 and open_time="'.date('Y-m-d',$time).' 09:00:00" and status="OPEN"')){
				$server_id = $row['id'];
			}else{
			...
			if(DB::exists('select id from fmg_lien_dau where round=1 and ld_server_id='.$server_id.'')){
				...
				$sql = '
					...
					ORDER BY
						fmg_clb_server.win DESC,fmg_clb_server.hang,fmg_clb_server.diem DESC,fmg_clb_server.power DESC
					LIMIT 0,'.$limit.'
				';
	...
	static function get_power_clb_temp($cau_thus){
		$total = 0;
		if($vong_dau_id=get_id_vong_dau_ket_thuc_gan_nhat(MUA_GIAI_ID)){
