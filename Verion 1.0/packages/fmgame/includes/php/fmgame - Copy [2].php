...
class FMGAME{
	...
	static function get_anh_hang_hlv($account_id){
		...
		}elseif($diem_kn>=1000 and $diem_kn<2500){	
	...
	static function can_transfer(){
		...
		//if($_SERVER['REMOTE_ADDR'] == '14.162.132.37' or $_SERVER['REMOTE_ADDR'] == '1.55.98.231')
		{
			return true;
		}
	...
	static function get_team($clb_id){
		$sql = '
			select 
				fmg_clb.id,fmg_clb.logo,fmg_clb.name,
				fmg_clb.stamina,
				fmg_clb.train,
				fmg_clb.first_time,
				fmg_clb.account_id,
	...
	static function schedule($server_id){
		...
		if($tong_clb>= MAX_TEAM and DB::fetch('select count(*) as total from fmg_schedule where server_id='.$server_id.'','total')<$tong_clb*($tong_clb-1)){
			...
			//if((date('N')==6 and date('H')<20))
			{
				...
				$start_time = strtotime(date('2016-03-21 00:00:00'));// dung de test
				$t = $start_time;
				$games = array();
