...
WRITTEN BY Khoand
******************************/
class ManagePromotion extends Module
{
	function ManagePromotion($row)
	{
		Module::Module($row);
		require_once 'db.php';
	...
	function copy_items()
	{
		if(User::can_edit(false,ANY_CATEGORY) and isset($_REQUEST['selected_ids']) and count($_REQUEST['selected_ids'])>0)
		{
			...
			$this->add_form(new CopyManagePromotionForm());
		}
		else
		{
	...
	function list_cmd()
	{
		...
		$this->add_form(new ListManagePromotionForm());
	}
	function add_cmd()
	{
		if(User::can_add(false,ANY_CATEGORY))
		{
			...
			$this->add_form(new EditManagePromotionForm());
		}
		else
		{
			Url::access_denied();
