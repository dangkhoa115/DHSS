...
<br>
<fieldset id="toolbar">
<form name="ManageCountry" method="post" action="?<?php echo htmlentities($_SERVER['QUERY_STRING']);?>">
<table cellpadding="6" cellspacing="0" width="100%" style="#width:99%;margin-top:8px;" border="1" bordercolor="#E7E7E7" align="center">
	...
	<!--LIST:items-->
	<?php $i++;?>
	<tr valign="middle" <?php Draw::hover(Portal::get_setting('crud_item_hover_bgcolor','#FFFFDD'));?> style="cursor:hand;<?php if($i%2){echo 'background-color:#F9F9F9';}?>" id="ManageCountry_tr_[[|items.id|]]">
		<td><?php echo $i;?></td>
		<td>[[|items.name|]]</td>
		<!--IF:cond(User::can_edit(false,ANY_CATEGORY))--><td align="center"><a href="<?php echo Url::build_current(array('cmd'=>'edit','id'=>[[=items.id=]]));?>"><img src="skins/default/images/buttons/edit.jpg"></a></td><!--/IF:cond-->
		<!--IF:cond1(User::can_delete(false,ANY_CATEGORY))--><td align="center"><a onclick="if(confirm('[[.are_you_want_to_delete.]]')){location='<?php echo Url::build_current(array('cmd'=>'delete','id'=>[[=items.id=]]));?>'}"><img src="skins/default/images/buttons/uncheck.gif"></a></td><!--/IF:cond1-->
	</tr>
	<!--/LIST:items-->
</table>	
