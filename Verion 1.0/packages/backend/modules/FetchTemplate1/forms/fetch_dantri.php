<?php
class FetchDanTriForm extends Form{
	function FetchDanTriForm(){
		Form::Form('FetchDanTriForm');		
	...
	function parse_content($url,$category_id){
		if(preg_match("/http\:\/\//",$url) and $content=file_get_contents($url)){	
			$pattern_name='/\<a id=\"ctl14_hplTitle\"\>([^\<]+)\<\/a\>/';
			$pattern_brief='<div class="introtext" style="padding: 6px 6px 6px 0;">';
			$pattern_description=' <span id="ctl14_ltrContent" class="bodyContent">';
			if(preg_match_all($pattern_name,$content,$match_name)){
				$name=$match_name[1][0];
			}
			$finish = 0;
			$brief = FetchTemplateDB::cut_string($content,$pattern_brief,'</div>');
			$description=FetchTemplateDB::cut_string($content,$pattern_description,'id=\'hidNextUsing\'/></span>');		
			if(isset($name)){
				...
				Url::redirect_current(array('cmd'=>'fetch_dantri'));
			}	
		}	
		else{
