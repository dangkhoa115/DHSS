<?php
class EditPolicyAdminForm extends Form
{
	function EditPolicyAdminForm()
	{
		Form::Form('EditPolicyAdminForm');
	...
	function on_submit()
	{
		...
		{
			if($facilities = Url::get('mi_policy_group') and is_array($facilities))
			{
				foreach($facilities as $key=>$facility)
				{
					if($facility['id'])
					{
						if($facility['name'])
						{
							DB::update_id('policies',array('name'=>$facility['name']),$facility['id']);
						}
					}else
					{
						DB::insert('policies',array('name'=>$facility['name']));
					}
				}
			}
			if($group_deleted_ids = Url::get('group_deleted_ids'))
			{
				...
				{
					DB::delete_id('policies',intval($value));
				}
			}
	...
	function draw()
	{
		...
		$sql = 'SELECT 
				...
				';
		$_REQUEST['mi_policy_group'] = DB::fetch_all($sql);
