		...
		<table cellpadding="6" cellspacing="0" width="100%" style="#width:99%;margin-top:8px;" align="center">
			...
			<tr bgcolor="#EFEFEF" valign="top" id="ManageItem_tr_[[|items.id|]]" <?php echo ([[=items.i=]]%2==0)?'bgcolor="#DAF2FE"':'bgcolor="#FFFFFF"';?>>
				...
				<td  align="left" style="border-right:1px solid #333;border-bottom:1px solid #333" bgcolor="#FFFFFF">
					<span class="label">Tên:</span><div id="name_[[|items.id|]]" style="font-size:14px;"><!--IF:cond(Url::get('act')=='select_item')--><a href="#" onclick="pickValue([[|items.id|]]);return false;" style="color:#F00;font-weight:bold;">[ Chọn ]</a> <!--/IF:cond--><span id="item_name_[[|items.id|]]">[[|items.name|]] (Vị trí: [[|items.position|]])</span></div></td>
        	...
        	<!--IF:cond([[=items.public_price=]])-->
          Gốc: <strong>[[|items.public_price|]]</strong><br />
          <!--/IF:cond-->
				  Bán: <strong>[[|items.price|]]</strong></td>
				<td align="left" nowrap="nowrap" width="1%"><span id="image_bound_[[|items.id|]]"><!--IF:cond([[=items.image_url=]])--><br /><img src="[[|items.image_url|]]" width="50" height="50" onclick="window.open('[[|items.image_url|]]','','width:800px,height:500px');"><!--ELSE--><!--/IF:cond--></span></td>
				...
				<td width="1%" align="left">User: <strong>[[|items.poster|]]</strong><br />Email: [[|items.email|]]</td>
                <!--IF:user_cond(User::can_admin(false,ANY_CATEGORY))-->
