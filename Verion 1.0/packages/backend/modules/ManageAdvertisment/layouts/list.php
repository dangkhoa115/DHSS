...
<fieldset id="toolbar">
	...
	<div id="toolbar-title">[[.manage_advertisment.]] <span style="font-size:16px;color:#0B55C4;">[ <?php echo Portal::language(Url::get('cmd','list'));?> ]</span></div>
	...
	<form name="ListManageAdvertismentForm" method="post" action="?<?php echo htmlentities($_SERVER['QUERY_STRING']);?>">
		...
		<thead>
			<tr valign="middle" bgcolor="#F0F0F0" style="line-height:20px">
				...
				<th width="10%" align="left"><a>[[.category_id.]]</a></th>
				<th width="5%" align="left"><a>[[.position.]]</a></th>
			...
			<!--LIST:items-->
				<tr bgcolor="<?php if((URL::get('just_edited_id',0)==[[=items.id=]]) or (is_numeric(array_search(MAP['items']['current']['id'],[[=just_edited_ids=]])))){ echo Portal::get_setting('crud_just_edited_item_bgcolor','#F7F7F7');} else {echo Portal::get_setting('crud_item_bgcolor','white');}?>" valign="middle" <?php Draw::hover(Portal::get_setting('crud_item_hover_bgcolor','#F7F7F7'));?> style="cursor:hand;<?php if($i%2){echo 'background-color:#F9F9F9';}?>" id="ManageAdvertisment_tr_[[|items.id|]]">
					...
					<td><a href="<?php echo Url::build_current(array('cmd'=>'advertisment','id'=>[[=items.id=]]));?>"><img src="skins/default/images/buttons/edit.gif"></a></td>
					<td>[[|items.id|]]</td>
					<td>[[|items.name|]]</td>
					<td align="center">
					<?php 
		...
		<tr>
			<td width="50%">
				[[.select.]]:&nbsp;
				<a onclick="select_all_checkbox(document.ListManageAdvertismentForm,'ManageAdvertisment',true,'<?php echo Portal::get_setting('crud_list_item_selected_bgcolor','#FFFFEC');?>','<?php echo Portal::get_setting('crud_item_bgcolor','white');?>');">[[.select_all.]]</a>&nbsp;| 
				<a onclick="select_all_checkbox(document.ListManageAdvertismentForm,'ManageAdvertisment',false,'<?php echo Portal::get_setting('crud_list_item_selected_bgcolor','#FFFFEC');?>','<?php echo Portal::get_setting('crud_item_bgcolor','white');?>');">[[.select_none.]]</a>|				<a onclick="select_all_checkbox(document.ListManageAdvertismentForm,'ManageAdvertisment',-1,'<?php echo Portal::get_setting('crud_list_item_selected_bgcolor','#FFFFEC');?>','<?php echo Portal::get_setting('crud_item_bgcolor','white');?>');">[[.select_invert.]]</a></td>
