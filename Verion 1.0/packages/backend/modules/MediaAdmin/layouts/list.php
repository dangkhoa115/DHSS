	  ...
	  <tbody>
		<tr>
		  ...
		  <?php if(User::can_delete(false,ANY_CATEGORY)){?><td id="toolbar-trash"  align="center"><a onclick="if(confirm('<?php echo Portal::language('are_you_sure_delete');?>')){if(check_selected()){make_cmd('delete')}else{alert('<?php echo Portal::language('You_must_select_atleast_item');?>');}}"> <span title="Trash"> </span> [[.Trash.]] </a> </td><?php }?>
  		  ...
  		  <?php if(User::can_admin(false,ANY_CATEGORY) and Url::sget('page')=='photo_admin'){?><td id="toolbar-param"  align="center"><a href="<?php echo Url::build_current(array('cmd'=>'make_slide'));?>#"> <span title="Make param"> </span> [[.Make_slide.]] </a> </td><?php }?>
	...
	<form name="MediaAdmin" method="post" action="?<?php echo htmlentities($_SERVER['QUERY_STRING']);?>">
		...
		<table cellpadding="6" cellspacing="0" width="100%" style="#width:99%;margin-top:8px;" border="1" bordercolor="#E7E7E7" align="center">
			<thead>
					...
					<th width="10%" align="left" nowrap><a>[[.category_id.]]</a></th>
				...
				<tr bgcolor="<?php if((URL::get('just_edited_id',0)==[[=items.id=]]) or (is_numeric(array_search(MAP['items']['current']['id'],[[=just_edited_ids=]])))){ echo Portal::get_setting('crud_just_edited_item_bgcolor','#FFFFDD');} else {echo Portal::get_setting('crud_item_bgcolor','white');}?>" valign="middle" <?php Draw::hover(Portal::get_setting('crud_item_hover_bgcolor','#FFFFDD'));?> style="cursor:hand;<?php if($i%2){echo 'background-color:#F9F9F9';}?>" id="MediaAdmin_tr_[[|items.id|]]">
					...
					{
						echo '<img src="'.[[=items.image_url=]].'" width="60" height="50" onerror="this.src=\'skins/default/images/no_image.gif\'">';
					}
					?>
					</td>
		...
		<table width="100%" cellpadding="6" cellspacing="0" style="background-color:#F0F0F0;border:1px solid #E7E7E7;height:8px;#width:99%" align="center">
			...
			<td width="48%" align="left">
				[[.select.]]:&nbsp;
				<a onclick="select_all_checkbox(document.MediaAdmin,'MediaAdmin',true,'<?php echo Portal::get_setting('crud_list_item_selected_bgcolor','#FFFFEC');?>','<?php echo Portal::get_setting('crud_item_bgcolor','white');?>');">[[.select_all.]]</a>&nbsp;
				<a onclick="select_all_checkbox(document.MediaAdmin,'MediaAdmin',false,'<?php echo Portal::get_setting('crud_list_item_selected_bgcolor','#FFFFEC');?>','<?php echo Portal::get_setting('crud_item_bgcolor','white');?>');">[[.select_none.]]</a>
				<a onclick="select_all_checkbox(document.MediaAdmin,'MediaAdmin',-1,'<?php echo Portal::get_setting('crud_list_item_selected_bgcolor','#FFFFEC');?>','<?php echo Portal::get_setting('crud_item_bgcolor','white');?>');">[[.select_invert.]]</a>		</td>
			<td width="18%">&nbsp;<a>[[.display.]]</a>
			  <select name="item_per_page" class="select" style="width:50px" size="1" onchange="document.MediaAdmin.submit( );" id="item_per_page" ></select>&nbsp;[[.of.]]&nbsp;[[|total|]]</td>
