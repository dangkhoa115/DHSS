...
<br>
<fieldset id="toolbar">
	<form name="SlideListMediaAdmin" method="post" action="?<?php echo htmlentities($_SERVER['QUERY_STRING']);?>">
		<a name="top_anchor"></a>		
		<table cellpadding="6" cellspacing="0" width="100%" style="#width:99%;margin-top:8px;" border="1" bordercolor="#E7E7E7" align="center">
			<thead>
				...
				<tr bgcolor="<?php if((URL::get('just_edited_id',0)==[[=items.id=]])){ echo Portal::get_setting('crud_just_edited_item_bgcolor','#FFFFDD');} else {echo Portal::get_setting('crud_item_bgcolor','white');}?>" valign="middle" <?php Draw::hover(Portal::get_setting('crud_item_hover_bgcolor','#FFFFDD'));?> style="cursor:hand;<?php if($i%2){echo 'background-color:#F9F9F9';}?>" id="SlideListMediaAdmin_tr_[[|items.id|]]">
					...
					<?php if(User::can_edit(false,ANY_CATEGORY))
					{?>
					<td width="3%" align="left" nowrap><a href="<?php echo Url::build_current(array('id'=>[[=items.id=]],'cmd'=>'make_slide'));?>"><img src="skins/default/images/buttons/edit.jpg"></a></td>
					<td width="4%" align="left" nowrap><a href="<?php echo Url::build_current(array('slide_id'=>[[=items.id=]],'cmd'=>'view_slide'));?>"><img src="skins/default/images/buttons/search.gif" width="20"></a></td>
					<?php }?>
				</tr>
				<!--/LIST:items-->
				</tbody>
			...
			<tr>
				<td width="48%" align="left">
					[[.select.]]:&nbsp;
					<a onclick="select_all_checkbox(document.SlideListMediaAdmin,'SlideListMediaAdmin',true,'<?php echo Portal::get_setting('crud_list_item_selected_bgcolor','#FFFFEC');?>','<?php echo Portal::get_setting('crud_item_bgcolor','white');?>');">[[.select_all.]]</a>&nbsp;
					<a onclick="select_all_checkbox(document.SlideListMediaAdmin,'SlideListMediaAdmin',false,'<?php echo Portal::get_setting('crud_list_item_selected_bgcolor','#FFFFEC');?>','<?php echo Portal::get_setting('crud_item_bgcolor','white');?>');">[[.select_none.]]</a>
					<a onclick="select_all_checkbox(document.SlideListMediaAdmin,'SlideListMediaAdmin',-1,'<?php echo Portal::get_setting('crud_list_item_selected_bgcolor','#FFFFEC');?>','<?php echo Portal::get_setting('crud_item_bgcolor','white');?>');">[[.select_invert.]]</a>		
				</td>
			  </tr>
			</table>			
	</table>
