<fieldset id="toolbar">
	...
	<div id="toolbar-personal">[[.manage_order.]]</div>
...
<form name="ContactList" method="post" action="?<?php echo htmlentities($_SERVER['QUERY_STRING']);?>">
	<table width="100%" cellpadding="3" cellspacing="0" border="1" style="border-collapse:collapse;" bordercolor="#CCCCCC">
		<tr bgcolor="#EBE8D6">
			<th width="1%">[[.stt.]]</th>
			<th width="50%" align="left">[[.full_name.]]</th>
			<th width="10%" align="right">[[.total_amount.]]</th>
		...
		<?php $i=1;?>
		<!--LIST:items-->
		<tr style="<?php if([[=items.checked=]]==1){?>font-weight:normal;<?php }else{?>font-weight:bold;<?php }?>cursor:pointer;"<?php if([[=items.checked=]]!=1){?>bgcolor="#FFFFFF"<?php }else{?>bgcolor="#F2F2F2"<?php }?>>
			<td align="center" onclick="window.location='<?php echo Url::build_current(array('cmd'=>'detail','id'=>[[=items.id=]]))?>'"><?php echo $i;?></td>
			<td onclick="window.location='<?php echo Url::build_current(array('cmd'=>'detail','id'=>[[=items.id=]]))?>'" valign="top" style="line-height:18px;;">[[|items.full_name|]]</td>
			<td align="right" valign="top" style="line-height:18px;;" onclick="window.location='<?php echo Url::build_current(array('cmd'=>'detail','id'=>[[=items.id=]]))?>'"><?php echo System::display_number([[=items.total_amount=]]);?></td>
		  <td onclick="window.location='<?php echo Url::build_current(array('cmd'=>'detail','id'=>[[=items.id=]]))?>'" nowrap="nowrap"><?php echo date('H:i d/m/Y',[[=items.time=]]);?></td>		
			...
			<td align="center"><a onclick="if(!confirm('[[.are_you_sure.]]?')){return false;}" href="<?php echo URL::build_current(array('cmd'=>'delete','id'=>User::encode_password([[=items.id=]])));?>" title="Delete"><img src="skins/default/images/icon/delete2.png" /></a></td>
		</tr>
		<?php $i++;?>
		<!--/LIST:items-->
	</table>
