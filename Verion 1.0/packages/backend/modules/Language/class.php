...
class Language extends Module
{
	...
	{
		$items =  DB::fetch_all('select * from language order by id ASC');
		$handler = fopen('cache/tables/language.php','w+');
		$content = '<?php $languages = '.var_export($items,1).';?>';
