...
class ManageContact extends Module
{
	function ManageContact($row)
	{
		...
		{
			if(Url::sget('page') == 'manage_contact')
			{
				switch(Url::get('cmd'))
				{
					case 'delete':
					...
					case 'detail':
						...
						$this->add_form(new ManageContactDetailForm());
						break;
					default:
	...
	{
		if(User::can_edit(false,ANY_CATEGORY) and  Url::get('id') and $contact = DB::fetch('select id,is_check from contact where id='.intval(Url::sget('id')).' and is_check=0'))
		{
