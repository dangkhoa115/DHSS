...
<fieldset id="toolbar">
	<div id="toolbar-title">
		[[.manage_game.]] <span style="font-size:16px;color:#0B55C4;">[ <?php echo Portal::language(Url::get('cmd','list'));?> ]</span>
	</div>
	...
	<form name="GameAdmin" method="post" action="?<?php echo htmlentities($_SERVER['QUERY_STRING']);?>">
		<a name="top_anchor"></a>		
			<table cellpadding="0" cellspacing="6" width="100%">
				<tr>
					...
					<td nowrap="nowrap">
					<select name="category_id" class="inputbox"  id="category_id" size="1" onchange="document.GameAdmin.submit();"></select>
					<!--<select name="author" id="author" class="inputbox" size="1" onchange="document.GameAdmin.submit();"></select> -->	
					<select name="status" id="status" class="inputbox" size="1" onchange="document.GameAdmin.submit();"></select>	
				  </td>
				</tr>
		</table>
		<table cellpadding="6" cellspacing="0" width="100%" style="#width:99%;margin-top:8px;" border="1" bordercolor="#E7E7E7" align="center">
			<thead>
					...
					<th width="10%" align="left" nowrap><a>[[.positon.]]</a><img src="skins/default/images/cms/menu/filesave.png" onclick="jQuery('#cmd').val('update_position');document.GameAdmin.submit();" style="cursor:pointer"></th>
				...
				<tbody>		
				<!--LIST:items-->	
				<tr bgcolor="<?php if((URL::get('just_edited_id',0)==[[=items.id=]]) or (is_numeric(array_search(MAP['items']['current']['id'],[[=just_edited_ids=]])))){ echo Portal::get_setting('crud_just_edited_item_bgcolor','#FFFFDD');} else {echo Portal::get_setting('crud_item_bgcolor','white');}?>" valign="middle" <?php Draw::hover(Portal::get_setting('crud_item_hover_bgcolor','#FFFFDD'));?> style="cursor:hand;<?php if([[=items.index=]]%2){echo 'background-color:#F9F9F9';}?>" id="GameAdmin_tr_[[|items.id|]]">
					<th width="1%" align="left" nowrap><a>[[|items.index|]]</a></th>
					<td width="1%"><input name="selected_ids[]" type="checkbox" value="[[|items.id|]]" onclick="select_checkbox(this.form,'GameAdmin',this,'<?php echo Portal::get_setting('crud_list_item_selected_bgcolor','#FFFFEC');?>','<?php echo Portal::get_setting('crud_item_bgcolor','white');?>');" id="GameAdmin_checkbox" <?php if(URL::get('cmd')=='delete') echo 'checked';?>></td >
					<td width="1%"><img src="[[|items.small_thumb_url|]]" width="50" /></td >
					<td  align="left"><a href="gaming/[[|items.category_name_id|]]/[[|items.name_id|]].html" target="_blank"> [[|items.name|]]</a>&nbsp;&nbsp;<?php if([[=items.total_comment=]]>0){?><a  style="color:#FF0000" href="<?php echo Url::build('manage_comment',array('item_id'=>[[=items.id=]]));?>">[[[|items.total_comment|]]]</a><img src="skins/default/images/cms/comment.gif"><?php }?></td>
					<td align="left" nowrap>[[|items.status|]]			  </td>
					<td align="center" nowrap><a href="<?php echo Url::build_current(array('id'=>[[=items.id=]],'cmd'=>'front_page'));?>"><img src="skins/default/images/cms/menu/publish[[|items.front_page|]].png"></a></td>
		...
		<table width="100%" cellpadding="6" cellspacing="0" style="background-color:#F0F0F0;border:1px solid #E7E7E7;height:8px;#width:99%" align="center">
			...
			<td width="48%" align="left">
				[[.select.]]:&nbsp;
				<a onclick="select_all_checkbox(document.GameAdmin,'GameAdmin',true,'<?php echo Portal::get_setting('crud_list_item_selected_bgcolor','#FFFFEC');?>','<?php echo Portal::get_setting('crud_item_bgcolor','white');?>');">[[.select_all.]]</a>&nbsp;
				<a onclick="select_all_checkbox(document.GameAdmin,'GameAdmin',false,'<?php echo Portal::get_setting('crud_list_item_selected_bgcolor','#FFFFEC');?>','<?php echo Portal::get_setting('crud_item_bgcolor','white');?>');">[[.select_none.]]</a>
				<a onclick="select_all_checkbox(document.GameAdmin,'GameAdmin',-1,'<?php echo Portal::get_setting('crud_list_item_selected_bgcolor','#FFFFEC');?>','<?php echo Portal::get_setting('crud_item_bgcolor','white');?>');">[[.select_invert.]]</a>		</td>
			<td width="18%">&nbsp;<a>[[.display.]]</a>
			  <select name="item_per_page" class="select" style="width:50px" size="1" onchange="document.GameAdmin.submit( );" id="item_per_page" ></select>&nbsp;[[.of.]]&nbsp;[[|total|]]</td>
