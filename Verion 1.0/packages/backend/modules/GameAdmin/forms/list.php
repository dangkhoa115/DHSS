<?php
class ListGameAdminForm extends Form
{
	function ListGameAdminForm()
	{
		Form::Form('ListGameAdminForm');
	...
	function draw()
	{
		...
		$total = GameAdminDB::get_total_item($cond);
		...
		$items = GameAdminDB::get_items($cond,$order_by,$item_per_page);		
		$item_per_page_list = array(20=>20,30=>30,50=>50,100=>100);
		$this->parse_layout('list',$this->just_edited_id+array(
			'items'=>$items,
			'paging'=>$paging,
			'total'=>$total,
			'category_id_list'=>array(Portal::language('select_category'))+String::get_list(GameAdminDB::get_category()),
			'status_list'=>array(Portal::language('select_status'))+$status,
			'author_list'=>array(Portal::language('select_user'))+String::get_list(GameAdminDB::get_user()),
