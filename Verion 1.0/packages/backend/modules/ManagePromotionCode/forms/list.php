<?php
class ListManagePromotionCodeForm extends Form
{
	function ListManagePromotionCodeForm()
	{
		Form::Form('ListManagePromotionCodeForm');
	...
	function draw()
	{
		...
		$item_per_page = 100;
		$total = ManagePromotionCodeDB::get_total_item($cond);
		$paging = paging($total,$item_per_page,10,false,'page_no',array('cmd'));
		$items = ManagePromotionCodeDB::get_items($cond,$item_per_page);
	...
	function save_position()
	{
		foreach($_REQUEST as $key=>$value)
		{
			...
			{
				DB::update_id('promotion_code',array('position'=>Url::get('position_'.$match[1])),$match[1]);
			}
		}
	...
	function delete()
	{
		if(isset($_REQUEST['selected_ids']) and count($_REQUEST['selected_ids'])>0)
		{
			...
			if($items = DB::fetch_all('SELECT * FROM promotion_code WHERE id IN ('.$selected_ids.')')){
				foreach($items as $key=>$item){
					DB::delete('promotion_code','id in ('.$selected_ids.')');
				}
			}
		}
		Url::redirect_current();
	...
	function get_condition()
	{
		$cond = '';
		if(Url::get('search')){
			$cond .= URL::get('search')? ' AND (promotion_code.rand_code LIKE "%'.addslashes(URL::sget('search')).'%")':'';
		}
		return $cond;
	}
}
