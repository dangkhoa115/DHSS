...
<table cellpadding="0" width="100%"><tr><td  class="form_title"><img src="skins/default/images/buttons/<?php echo $action;?>_button.gif" align="absmiddle" alt=""/><?php echo $title;?></td><td class="form_title_button"><a onclick="EditModuleAdminForm.submit();"><img src="skins/default/images/buttons/save_button.gif" style="text-align:center"/><br /><?php echo Portal::language('save');?></a></td><?php if($action=='edit'){?><td class="form_title_button"><a onclick="location='<?php echo URL::build_current(array('cmd'=>($this->map['type']=='CONTENT')?'edit_content':(($this->map['type']=='HTML')?'edit_html':'edit_code'),'id'));?>';"><img width="30px" src="skins/default/images/buttons/edit_button.gif" style="text-align:center"/><br /><?php echo Portal::language('edit_code');?></a></td><?php }?>
		<td class="form_title_button">
			<a onclick="location='<?php echo URL::build_current();?>';"><img src="skins/default/images/buttons/go_back_button.gif"/><br /><?php echo Portal::language('back');?></a></td>
		<?php if($action=='edit'){?><td class="form_title_button">
			<a ) onclick="location='<?php echo URL::build_current(array('cmd'=>'delete','id'));?>';"><img src="skins/default/images/buttons/delete_button.gif" alt=""/><br /><?php echo Portal::language('Delete');?></a></td><?php }?>
		<td class="form_title_button">
	...
	<table cellspacing="0" width="100%"><tr><td>
		...
		<div class="form_input">
				<select  name="package_id" id="package_id"><?php
					...
					?><script type="text/javascript">getId('package_id').value = "<?php echo addslashes(URL::get('package_id',isset($this->map['package_id'])?$this->map['package_id']:''));?>";</script>
	</select>
		</div>
		...
		<div id="action_info" <?php if(URL::get('type')!='PLUGIN' or URL::get('type')!='WRAPPER')echo 'style="display:none"';?>>
			<div id="plugin_action_info" <?php if(URL::get('type')!='PLUGIN')echo 'style="display:none"';?>>
				...
				<select  name="action" id="action"><?php
					...
					?><script type="text/javascript">getId('action').value = "<?php echo addslashes(URL::get('action',isset($this->map['action'])?$this->map['action']:''));?>";</script>
	</select> </div>
			<?php echo Portal::language('on');?><select  name="action_module_id" id="action_module_id"><?php
					...
					?><script type="text/javascript">getId('action_module_id').value = "<?php echo addslashes(URL::get('action_module_id',isset($this->map['action_module_id'])?$this->map['action_module_id']:''));?>";</script>
	</select>
