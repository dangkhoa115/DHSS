...
<div class="fm-bound">
	...
	<table width="99%" class="fm-menu" border="1" bordercolor="#CCCCCC" cellpadding="0" cellspacing="0">
		<tr>
			<td align="center">
				<div id="back">
					<img style="width:25px; height:25px" src="skins/default/images/tree_files/up.png" /><br />
					Up
				</div>
				<div id="create_folder" rel="<?php echo $this->map['current'];?>">
						<img style="width:25px; height:25px" src="skins/default/images/tree_files/create_folder.png" /><br />
						Tạo thư mục
					</div>
			...
			<td><div class="path">/upload<?php echo $this->map['current'];?></div></td>
		</tr>
		<tr>
			<td width="50%" align="center">
				URL : <input type="text" readonly id="url" />
				<input type="button" value="Copy" id="returnValue"  />
			</td>
	...
	<?php
					...
					{
						foreach($this->map['list'] as $key1=>&$item1)
						{
							if($key1!='current')
							{
								$this->map['list']['current'] = &$item1;?>
		<?php 
				if(($this->map['list']['current']['type']=='folder'))
				{?>
			<div class="fm-folder" rel="<?php echo $this->map['list']['current']['name'];?>">
				<img src="skins/default/images/tree_files/directory.png" />
				<div class="fm-name"><?php echo $this->map['list']['current']['name'];?></div>
			</div>
				...
				?>
		<?php 
				if(($this->map['list']['current']['type']=='file'))
				{?>
			<div class="fm-file" path="<?php echo $this->map['list']['current']['src'];?>" rel="<?php echo $this->map['list']['current']['name'];?>">
				<?php 
				if(($this->map['list']['current']['ext']=='image'))
				{?>
				<img src="<?php echo $this->map['list']['current']['src'];?>" />
				 <?php }else{ ?>
				<img src="skins/default/images/tree_files/<?php echo $this->map['list']['current']['ext'];?>.png" />
...
<div class="fm-menu-command" id="fm-menu-command">
	<div id="delete"><a href="javascript:void(0)"><?php echo Portal::language('delete');?></a></div>
	<div id="rename"><a href="javascript:void(0)"><?php echo Portal::language('rename');?></a></div>
</div>
	<input type="hidden" value="<?php echo $this->map['current'];?>" name="currentFolder" id="currentFolder" />
...
jQuery(document).ready(function(){
	...
	jQuery('#returnValue').click(function(){
		...
		window.prompt("Copy đường dẫn ảnh: Ctrl+C, Enter", jQuery('#url').val());
		jQuery('#url').select();
	});
	...
	jQuery('#delete').click(function(){
		//alert('<?php echo Portal::language('this_function_is_locking');?>');
		if(confirm('<?php echo Portal::language('Are_you_want_to_delete_this_file');?>'))
		{
	...
	jQuery('#upload-btn').click(function(){
		...
		{
			alert('<?php echo Portal::language('You_must_select_file_before_upload');?>');
		}
	});
