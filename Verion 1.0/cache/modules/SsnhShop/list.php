<div class="content-r shop">
  ...
  <div class="detail col-md-8">
    <ul class="nav nav-tabs" role="tablist">
     ...
     <li role="presentation"<?php echo (Url::get('tab')=='used_item')?' class="active"':'';?>>
...
function buyItem(itemId,quantity){
	if(quantity<1){
		alert('Bạn vui lòng chọn số lượng item cần mua');
		return false;	
	}else{
		jQuery.ajax({
			method: "POST",
			...
			success: function(content){
				if(content == 'true'){
					alert('Bạn đã mua item thành công!');
					location.reload();
				}else if(content == 'false'){
					alert('Số lượng item bạn mua lớn hơn số lượng item tối đa bạn được mua');
				} else if (content == 'not_enough_igold'){
					alert('Số iGold của bạn không đủ để mua item. Vui lòng nạp thêm iGold để mua!');
					return false;
				}
			},
			error: function(){
...
function useItem(itemId,type){
	...
	*/
	switch (type){
		case 1:
			window.location = 'vong_quay_may_man.html?act=quay';
			break;			
		case 2:
			break;
		case 3:
			window.location = 'tham-gia-binh-chon/do/x2.html';
			break;
		case 4:
			window.location = 'tham-gia-binh-chon.html?item_id='+itemId;
			break;
		case 5:
