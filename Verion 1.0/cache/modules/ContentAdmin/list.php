	...
	<form name="ContentAdmin" method="post" action="?<?php echo htmlentities($_SERVER['QUERY_STRING']);?>">
			...
			<table cellpadding="0" cellspacing="6" width="100%">
				<tr>
					<td width="100%">
						...
						<button onclick="document.ContentAdmin.submit();">&nbsp;<?php echo Portal::language('Go');?>&nbsp;</button>
					</td>
					<td nowrap="nowrap">
					<select  name="author" id="author" class="inputbox" size="1" onchange="document.ContentAdmin.submit();"><?php
					...
					<select  name="status" id="status" class="inputbox" size="1" onchange="document.ContentAdmin.submit();"><?php
		...
		<table cellpadding="6" cellspacing="0" width="100%" style="#width:99%;margin-top:8px;" border="1" bordercolor="#E7E7E7" align="center">
			<thead>
					...
					<th width="5%" align="left" nowrap><a><?php echo Portal::language('category');?></a></th>
					<th width="25%" align="left" nowrap><a><?php echo Portal::language('path_link');?></a></th>
					<th width="5%" align="left" nowrap><a><?php echo Portal::language('status');?></a></th>
					<th width="5%" align="left" nowrap><a><?php echo Portal::language('user_id');?></a></th>
					<th width="5%" align="left" nowrap><a><?php echo Portal::language('date');?></a></th>
				...
				<tr bgcolor="<?php if((URL::get('just_edited_id',0)==$this->map['items']['current']['id']) or (is_numeric(array_search($this->map['items']['current']['id'],$this->map['just_edited_ids'])))){ echo Portal::get_setting('crud_just_edited_item_bgcolor','#FFFFDD');} else {echo Portal::get_setting('crud_item_bgcolor','white');}?>" valign="middle" <?php Draw::hover(Portal::get_setting('crud_item_hover_bgcolor','#FFFFDD'));?> style="cursor:hand;<?php if($i%2){echo 'background-color:#F9F9F9';}?>" id="ContentAdmin_tr_<?php echo $this->map['items']['current']['id'];?>">
					...
					<td align="left" nowrap><?php echo $this->map['items']['current']['file'];?></td>
					...
					<?php if(User::can_edit(false,ANY_CATEGORY))
					{?>
					<td align="left" nowrap width="9%"><a href="<?php echo Url::build_current(array('id'=>$this->map['items']['current']['id'],'cmd'=>'edit'));?>"><img src="skins/default/images/buttons/edit.jpg"></a></td>
					<?php }?>
				</tr>
		...
		<tr>
			<td width="48%" align="left">
				...
				<a onclick="select_all_checkbox(document.ContentAdmin,'ContentAdmin',true,'<?php echo Portal::get_setting('crud_list_item_selected_bgcolor','#FFFFEC');?>','<?php echo Portal::get_setting('crud_item_bgcolor','white');?>');"><?php echo Portal::language('select_all');?></a>&nbsp;
				<a onclick="select_all_checkbox(document.ContentAdmin,'ContentAdmin',false,'<?php echo Portal::get_setting('crud_list_item_selected_bgcolor','#FFFFEC');?>','<?php echo Portal::get_setting('crud_item_bgcolor','white');?>');"><?php echo Portal::language('select_none');?></a>
				<a onclick="select_all_checkbox(document.ContentAdmin,'ContentAdmin',-1,'<?php echo Portal::get_setting('crud_list_item_selected_bgcolor','#FFFFEC');?>','<?php echo Portal::get_setting('crud_item_bgcolor','white');?>');"><?php echo Portal::language('select_invert');?></a>		</td>
			<td width="18%">&nbsp;<a><?php echo Portal::language('display');?></a>
			  <select  name="item_per_page" class="select" style="width:50px" size="1" onchange="document.ContentAdmin.submit( );" id="item_per_page" ><?php
