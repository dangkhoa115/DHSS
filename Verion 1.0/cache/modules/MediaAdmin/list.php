	  ...
	  <tbody>
		<tr>
		  ...
		  <?php if(User::can_delete(false,ANY_CATEGORY)){?><td id="toolbar-trash"  align="center"><a onclick="if(confirm('<?php echo Portal::language('are_you_sure_delete');?>')){if(check_selected()){make_cmd('delete')}else{alert('<?php echo Portal::language('You_must_select_atleast_item');?>');}}"> <span title="Trash"> </span> <?php echo Portal::language('Trash');?> </a> </td><?php }?>
   		  <?php if(User::can_view(false,ANY_CATEGORY)){?><td id="toolbar-list"  align="center"><a  href="<?php echo Url::build_current(array('cmd'=>'slide_list'));?>"> <span title="<?php echo Portal::language('Slide_list');?>"> </span> <?php echo Portal::language('Slide_list');?> </a> </td><?php }?>
  		  <?php if(User::can_admin(false,ANY_CATEGORY) and Url::sget('page')=='photo_admin'){?><td id="toolbar-param"  align="center"><a href="<?php echo Url::build_current(array('cmd'=>'make_slide'));?>#"> <span title="Make param"> </span> <?php echo Portal::language('Make_slide');?> </a> </td><?php }?>
...
<br>
<fieldset id="toolbar">
	<form name="MediaAdmin" method="post" action="?<?php echo htmlentities($_SERVER['QUERY_STRING']);?>">
		<a name="top_anchor"></a>		
			<table cellpadding="0" cellspacing="6" width="100%">
				<tr>
					<td width="100%">
						...
						<button onclick="document.MediaAdmin.submit();">&nbsp;<?php echo Portal::language('Go');?>&nbsp;</button>
					</td>
					<td nowrap="nowrap">	
					<select  name="category_id" id="category_id" class="inputbox" size="1" onchange="document.MediaAdmin.submit();"><?php
					...
					<select  name="status" id="status" class="inputbox" size="1" onchange="document.MediaAdmin.submit();"><?php
		...
		<table cellpadding="6" cellspacing="0" width="100%" style="#width:99%;margin-top:8px;" border="1" bordercolor="#E7E7E7" align="center">
			<thead>
					...
					<th width="31%" align="left" nowrap><a><?php echo Portal::language('name');?></a></th>
					<th width="5%" align="left" nowrap><a><?php echo Portal::language('image_url');?></a></th>
					<th width="10%" align="left" nowrap><a><?php echo Portal::language('category_id');?></a></th>
					<th width="2%" align="left" nowrap>Video</th>
					...
					<th width="4%" align="left" nowrap>Xem</th>
		...
		<table width="100%" cellpadding="6" cellspacing="0" style="background-color:#F0F0F0;border:1px solid #E7E7E7;height:8px;#width:99%" align="center">
			...
			<td width="48%" align="left">
				...
				<a onclick="select_all_checkbox(document.MediaAdmin,'MediaAdmin',true,'<?php echo Portal::get_setting('crud_list_item_selected_bgcolor','#FFFFEC');?>','<?php echo Portal::get_setting('crud_item_bgcolor','white');?>');"><?php echo Portal::language('select_all');?></a>&nbsp;
				<a onclick="select_all_checkbox(document.MediaAdmin,'MediaAdmin',false,'<?php echo Portal::get_setting('crud_list_item_selected_bgcolor','#FFFFEC');?>','<?php echo Portal::get_setting('crud_item_bgcolor','white');?>');"><?php echo Portal::language('select_none');?></a>
				<a onclick="select_all_checkbox(document.MediaAdmin,'MediaAdmin',-1,'<?php echo Portal::get_setting('crud_list_item_selected_bgcolor','#FFFFEC');?>','<?php echo Portal::get_setting('crud_item_bgcolor','white');?>');"><?php echo Portal::language('select_invert');?></a>		</td>
			<td width="18%">&nbsp;<a><?php echo Portal::language('display');?></a>
			  <select  name="item_per_page" class="select" style="width:50px" size="1" onchange="document.MediaAdmin.submit( );" id="item_per_page" ><?php
